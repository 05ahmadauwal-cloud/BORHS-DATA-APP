...
const NET = {
  ...
  airtel:  { display: 'Airtel',  bg: '#E11B22', fg: '#ffffff', short: 'Airtel' },
...
const TESTIMONIALS = [
  { initial: 'AO', color: '#FFCC00', name: 'Adebayo Okonkwo', role: 'Data Reseller · Lagos',
    text: 'Eight months, hundreds of customers, zero failed transactions. My customers refuse to let me switch platforms. BORHS just works.' },
  { initial: 'CE', color: '#E11B22', name: 'Chioma Eze', role: 'University Student · Enugu',
    text: "1GB for ₦285 sounds impossible but it's real and instant. I've tried every VTU platform in Nigeria and nothing touches BORHS." },
...
const ACTIVITY = [
  { name: 'Emeka O.',    city: 'Lagos',         item: '1GB MTN SME',       price: '₦285',   ago: '2m ago' },
  { name: 'Fatima A.',   city: 'Kano',          item: '2GB Airtel',        price: '₦580',   ago: '5m ago' },
  { name: 'Chukwudi N.', city: 'Onitsha',       item: 'Electricity token', price: '₦2,000', ago: '8m ago' },
  { name: 'Aisha B.',    city: 'Abuja',         item: '2GB MTN SME',       price: '₦510',   ago: '11m ago' },
  { name: 'Tunde F.',    city: 'Ibadan',        item: 'DStv Compact',      price: '₦9,000', ago: '14m ago' },
  { name: 'Grace E.',    city: 'Port Harcourt', item: '2GB Glo Gifting',   price: '₦550',   ago: '18m ago' },
];
  ...
  return (
    <div style={{ position: 'relative', width: 320, margin: '0 auto', userSelect: 'none' }}>
      ...
      <div style={{
        ...
        border: '1px solid rgba(51,65,85,0.65)', borderRadius: 24, padding: 26,
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#64748b', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>Wallet Balance</div>
          ...
          <div style={{ width: 42, height: 42, borderRadius: 13, flexShrink: 0, background: 'linear-gradient(135deg, rgba(37,99,235,0.22) 0%, rgba(37,99,235,0.08) 100%)', border: '1px solid rgba(37,99,235,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Wifi size={19} style={{ color: '#60a5fa' }} />
          </div>
        </div>

        {/* Recent transaction */}
        <div style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.14)', borderRadius: 14, padding: '14px 16px', marginBottom: 18 }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: '#475569', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>Last Purchase</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, background: net.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 900, color: net.fg }}>
              {net.short}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0', marginBottom: 3 }}>{planLabel}</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#10b981', display: 'flex', alignItems: 'center', gap: 4 }}>
                <CheckCircle size={11} style={{ color: '#10b981' }} /> Delivered in 4 seconds
              </div>
            </div>
            <div style={{ fontSize: 16, fontWeight: 900, color: '#f1f5f9' }}>{price}</div>
          </div>
        </div>

        {/* Networks */}
        <div>
          <div style={{ fontSize: 10, fontWeight: 600, color: '#475569', letterSpacing: '0.04em', marginBottom: 10 }}>Available Networks</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 7 }}>
            {Object.entries(NET).map(([key, n]) => (
              <div key={key} style={{ height: 32, borderRadius: 9, background: n.bg + '1A', border: `1px solid ${n.bg}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 900, color: n.bg === '#FFCC00' ? '#a89200' : n.fg === '#fff' ? n.bg : n.fg }}>
                {n.short}
              </div>
            ))}
          </div>
      ...
      {/* Delivered badge */}
      <div style={{ position: 'absolute', top: -16, right: -16, background: 'linear-gradient(135deg, rgba(16,185,129,0.97), rgba(5,150,105,0.97))', borderRadius: 16, padding: '10px 14px', boxShadow: '0 8px 28px rgba(16,185,129,0.4), 0 0 0 1px rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', gap: 9 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.75)', marginTop: 1 }}>{size} {net.display} · {price}</div>
        </div>
      </div>

      {/* Speed badge */}
      <div style={{ position: 'absolute', bottom: -16, left: -16, background: 'rgba(9,11,17,0.96)', border: '1px solid rgba(37,99,235,0.3)', borderRadius: 16, padding: '10px 14px', boxShadow: '0 8px 28px rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', gap: 9 }}>
...
export default function Home() {
  ...
  // Cheapest plan per network (for ticker + hero card)
  const byNetwork = {};
  plans.forEach((p) => {
  ...
  // Ticker items — all real plans, 4 clones for seamless loop
  ...
  // Pricing table — up to 8 plans, pick spread across networks
  const tablePlans = plans.slice(0, 8);

  // Stats display
  const userCount = statsRes?.userCount ?? null;
  const txCount = statsRes?.txCount ?? null;
  const volumeProcessed = statsRes?.volumeProcessed ?? null;

  const statItems = [
    {
      value: userCount !== null ? `${userCount}+` : '—',
      label: 'Active Users', sub: 'and growing', icon: Users, color: '#3b82f6',
    },
    {
      value: volumeProcessed !== null ? formatVolume(volumeProcessed) + '+' : '—',
      label: 'Processed', sub: 'in transactions', icon: TrendingUp, color: '#10b981',
    },
  ...
  return (
    <div style={{ overflowX: 'hidden' }}>
      <style>{`
        ...
        @keyframes livePulse { 0%,100% { opacity:1; box-shadow:0 0 0 0 rgba(16,185,129,0.5); } 50% { opacity:.8; box-shadow:0 0 0 6px rgba(16,185,129,0); } }
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div className="text-center lg:text-left">
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 30, flexWrap: 'wrap' }} className="lg:justify-start">
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(16,185,129,0.09)', border: '1px solid rgba(16,185,129,0.22)', borderRadius: 999, padding: '6px 14px', fontSize: 12, fontWeight: 700, color: '#10b981' }}>
                  <span className="live-dot" style={{ width: 7, height: 7, borderRadius: '50%', background: '#10b981', display: 'block', flexShrink: 0 }} />
                  All systems live
                </span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(37,99,235,0.09)', border: '1px solid rgba(37,99,235,0.2)', borderRadius: 999, padding: '6px 14px', fontSize: 12, fontWeight: 700, color: '#60a5fa' }}>
                  <Users size={11} />
                  {userCount !== null ? `${userCount}+ Active Users` : 'Growing Community'}
                </span>
              </div>

              <h1 style={{ fontSize: 'clamp(38px, 6vw, 68px)', fontWeight: 900, lineHeight: 1.05, letterSpacing: '-0.02em', color: 'var(--text-primary)', marginBottom: 22 }}>
                Nigeria's Cheapest
                <br />
                <span style={{ background: 'linear-gradient(90deg, #3b82f6 0%, #10b981 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                  Data &amp; Bills
                </span>
                <br />
                Platform
              ...
              <p style={{ fontSize: 'clamp(15px, 1.8vw, 18px)', color: 'var(--text-muted)', lineHeight: 1.72, marginBottom: 36, maxWidth: 500 }} className="mx-auto lg:mx-0">
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 32 }} className="lg:justify-start">
                <Link to="/register" className="btn-primary btn-lg" style={{ gap: 8, fontSize: 15, padding: '14px 30px', boxShadow: '0 8px 28px rgba(37,99,235,0.38)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 18px', justifyContent: 'center', fontSize: 13, color: 'var(--text-faint)' }} className="lg:justify-start">
                {['Free to join', 'No hidden fees', 'Instant delivery', 'Secure payments'].map((t) => (
                  <span key={t} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <CheckCircle size={13} style={{ color: '#10b981', flexShrink: 0 }} /> {t}
                  </span>
                ))}
              </div>
            </div>
          ...
          <div className="ticker-track">
            {tickerItems.map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 20px', flexShrink: 0 }}>
                <div style={{ width: 26, height: 26, borderRadius: 7, background: item.bg, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 900, color: item.fg }}>
                  {item.short}
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px" style={{ background: 'var(--border)', borderRadius: 20, overflow: 'hidden', border: '1px solid var(--border)' }}>
            {statItems.map((s) => (
              <div key={s.label} style={{ background: 'var(--bg-surface)', padding: '32px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ width: 42, height: 42, borderRadius: 12, background: s.color + '18', border: `1px solid ${s.color}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
      ...
      <section style={{ padding: '40px 24px 100px', background: 'var(--bg-surface)', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {SERVICES.map((s) => (
              <Link key={s.label} to={s.to} style={{ display: 'flex', flexDirection: 'column', padding: 28, borderRadius: 20, textDecoration: 'none', background: 'var(--bg-card)', border: '1px solid var(--border)', transition: 'transform 0.18s ease, box-shadow 0.18s ease, border-color 0.18s ease', position: 'relative', overflow: 'hidden' }}
                onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.borderColor = s.accent + '55'; e.currentTarget.style.boxShadow = `0 20px 48px rgba(0,0,0,0.25), 0 0 0 1px ${s.accent}22`; }}
              ...
              >
                ...
                {s.tag && (
                  <div style={{ position: 'absolute', top: 20, right: 20, background: s.accent + '1A', border: `1px solid ${s.accent}30`, borderRadius: 999, padding: '3px 10px', fontSize: 9, fontWeight: 800, color: s.accent, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{s.tag}</div>
                )}
                <div style={{ width: 50, height: 50, borderRadius: 15, background: s.accent + '18', border: `1px solid ${s.accent}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 22 }}>
          ...
          <div style={{ border: '1px solid var(--border)', borderRadius: 20, overflow: 'hidden' }}>
            ...
            {tablePlans.length === 0 ? (
              ...
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', padding: '17px 20px', borderBottom: i < 5 ? '1px solid var(--border)' : 'none', gap: 8 }}>
                  {[60, 48, 48, 52].map((w, j) => (
                    <div key={j} style={{ height: 14, width: w, borderRadius: 6, background: 'var(--bg-elevated)', opacity: 0.6 }} />
                  ))}
                </div>
              ))
            ) : (
              tablePlans.map((p, i) => {
                ...
                return (
                  <div key={p._id || i} style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', padding: '17px 20px', alignItems: 'center', borderBottom: i < tablePlans.length - 1 ? '1px solid var(--border)' : 'none', background: p.dataType === 'sme' && p.dataSize === '1GB' ? 'rgba(37,99,235,0.04)' : 'transparent' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 8, background: n.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 900, color: n.fg, flexShrink: 0 }}>{n.short}</div>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-secondary)' }}>{n.display}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-primary)' }}>{p.dataSize}</span>
                      <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-faint)', textTransform: 'capitalize' }}>{p.dataType}</span>
                    </div>
                    <div style={{ fontSize: 17, fontWeight: 900, color: '#10b981' }}>₦{p.sellingPrice.toLocaleString()}</div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-faint)' }}>{p.validity || '30 Days'}</div>
                  </div>
                );
              })
            )}
      ...
      <section style={{ padding: '80px 24px 100px', background: 'var(--bg-surface)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {STEPS.map((step, i) => (
              <div key={step.n} style={{ padding: '32px 26px 28px', borderRadius: 22, background: 'var(--bg-card)', border: '1px solid var(--border)', position: 'relative', textAlign: 'center' }}>
                <div style={{ position: 'absolute', top: 18, right: 18, fontSize: 10, fontWeight: 900, color: 'var(--text-faint)', background: 'var(--bg-elevated)', borderRadius: 8, padding: '3px 9px' }}>0{step.n}</div>
                <div style={{ width: 56, height: 56, borderRadius: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 22px', background: i === 1 ? 'linear-gradient(135deg, #2563eb, #1d4ed8)' : 'var(--bg-elevated)', border: i === 1 ? 'none' : '1px solid var(--border)', boxShadow: i === 1 ? '0 12px 36px rgba(37,99,235,0.45)' : 'none' }}>
      ...
      <section style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Live Activity Feed</span>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {ACTIVITY.map((a, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 14, background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                <div style={{ width: 36, height: 36, borderRadius: 11, flexShrink: 0, background: 'rgba(37,99,235,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 900, color: '#60a5fa' }}>
                  {a.name[0]}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.name} · {a.city}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 2 }}>{a.item}{a.price ? ` · ${a.price}` : ''}</div>
                </div>
      ...
      <section style={{ padding: '80px 24px 100px', background: 'var(--bg-surface)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 900, color: 'var(--text-primary)', marginBottom: 12 }}>Built for Real Nigerians</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map((f) => (
              <div key={f.title} style={{ padding: '24px 22px', borderRadius: 18, background: 'var(--bg-card)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 13, background: 'rgba(37,99,235,0.1)', border: '1px solid rgba(37,99,235,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <section style={{ padding: '100px 24px' }}>
        <div style={{ maxWidth: 1060, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <h2 style={{ fontSize: 'clamp(26px, 3.5vw, 42px)', fontWeight: 900, color: 'var(--text-primary)', marginBottom: 10 }}>Real People. Real Results.</h2>
            <p style={{ fontSize: 15, color: 'var(--text-muted)' }}>Here's what our users say about BORHS Data.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {TESTIMONIALS.map((t, i) => (
              <div key={t.name} style={{ padding: 28, borderRadius: 22, background: i === 1 ? 'linear-gradient(150deg, rgba(37,99,235,0.09) 0%, var(--bg-surface) 100%)' : 'var(--bg-surface)', border: i === 1 ? '1px solid rgba(37,99,235,0.28)' : '1px solid var(--border)', boxShadow: i === 1 ? '0 16px 48px rgba(37,99,235,0.13)' : 'none', display: 'flex', flexDirection: 'column', gap: 18 }}>
                <Stars />
                <p style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.72, flex: 1 }}>"{t.text}"</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 13, flexShrink: 0, background: t.color + '28', border: `2px solid ${t.color}45`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 900, color: t.color === '#FFCC00' ? '#a89200' : t.color }}>
                    {t.initial}
                  </div>
                  <div>
      ...
      <section style={{ padding: '0 24px 100px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ borderRadius: 28, padding: '52px 48px', overflow: 'hidden', background: 'linear-gradient(135deg, rgba(234,179,8,0.1) 0%, rgba(234,179,8,0.03) 100%)', border: '1px solid rgba(234,179,8,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 36, position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(234,179,8,0.12)', border: '1px solid rgba(234,179,8,0.25)', borderRadius: 999, padding: '5px 14px', fontSize: 11, fontWeight: 800, color: '#fbbf24', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 18 }}>
                <Sparkles size={11} /> Agent Program
              </div>
              <h3 style={{ fontSize: 'clamp(22px, 3vw, 34px)', fontWeight: 900, color: 'var(--text-primary)', marginBottom: 12 }}>Turn Data Selling into Real Income</h3>
              <p style={{ fontSize: 15, color: 'var(--text-muted)', maxWidth: 440, lineHeight: 1.68, marginBottom: 24 }}>
                ...
                <strong style={{ color: 'var(--text-primary)' }}>5% commission</strong>{' '}
                on every purchase from your entire downline — credited instantly to your wallet.
              </p>
              <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap' }}>
                {[{ label: '5% Commission', sub: 'Per transaction' }, { label: 'No Team Cap', sub: 'Unlimited downlines' }, { label: 'Instant Credit', sub: 'Wallet payouts' }].map((b) => (
                  <div key={b.label}>
            ...
            <Link to="/become-agent" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '14px 28px', borderRadius: 14, fontSize: 15, fontWeight: 800, background: 'linear-gradient(135deg, #eab308, #ca8a04)', color: '#1a1200', textDecoration: 'none', flexShrink: 0, boxShadow: '0 8px 28px rgba(234,179,8,0.38)' }}>
      ...
      <section style={{ padding: '0 24px 100px' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          <div style={{ borderRadius: 32, padding: '80px 48px', textAlign: 'center', background: 'linear-gradient(150deg, rgba(37,99,235,0.18) 0%, rgba(15,23,42,0.95) 48%, rgba(16,185,129,0.1) 100%)', border: '1px solid rgba(37,99,235,0.22)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(37,99,235,0.12)', border: '1px solid rgba(37,99,235,0.25)', borderRadius: 999, padding: '6px 16px', fontSize: 11, fontWeight: 800, color: '#60a5fa', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 26 }}>
                <Sparkles size={11} /> Free for everyone, always
              </div>
              <h2 style={{ fontSize: 'clamp(30px, 5vw, 54px)', fontWeight: 900, color: 'var(--text-primary)', lineHeight: 1.08, letterSpacing: '-0.02em', marginBottom: 18 }}>
                Start Saving Money
                <br />
                <span style={{ background: 'linear-gradient(90deg, #3b82f6, #10b981)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Right Now</span>
              </h2>
              <p style={{ fontSize: 17, color: 'var(--text-muted)', maxWidth: 460, margin: '0 auto 40px', lineHeight: 1.68 }}>
                {userCount !== null ? `Join ${userCount}+ Nigerians` : 'Join Nigerians'} who never overpay for data again. Sign up takes 60 seconds.
              </p>
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                ...
                <Link to="/login" className="btn-secondary btn-lg" style={{ fontSize: 15, padding: '14px 28px' }}>Sign In</Link>
              </div>
            </div>
          </div>
        </div>
