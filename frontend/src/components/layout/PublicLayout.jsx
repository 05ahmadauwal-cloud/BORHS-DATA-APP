    ...
    <div className="public-site min-h-screen bg-[#fafaf7] text-[#103c2d]">
      <header className="sticky top-0 z-40 border-b border-[#103c2d]/10 bg-[#fafaf7]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
          <Link to="/" className="shrink-0"><img src="/logo.svg" alt="BORHS Data" className="h-11 w-auto" /></Link>
      ...
      <div className={`fixed inset-0 z-50 transition md:hidden ${menuOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        ...
        <aside className={`absolute right-0 top-0 flex h-full w-[86%] max-w-sm flex-col bg-[#fafaf7] p-6 shadow-2xl transition-transform duration-300 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex items-center justify-between"><img src="/logo.svg" alt="BORHS Data" className="h-10 w-auto" /><button onClick={() => setMenuOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-full bg-[#eef1e9]" aria-label="Close"><X size={18} /></button></div>
      ...
      <footer className="mx-4 mb-4 mt-20 rounded-[2rem] bg-[#073b2a] px-6 py-12 text-white sm:mx-6 sm:px-10 lg:mx-auto lg:max-w-7xl">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1fr]"><div><img src="/logo.svg" alt="BORHS Data" className="h-12 w-auto rounded-xl bg-white p-1" /><p className="mt-5 max-w-xs text-sm leading-6 text-white/65">One dependable balance for data, airtime, bills and everyday Nigerian payments.</p></div>{[
